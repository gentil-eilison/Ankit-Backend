...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> str | None:
        ...
        for name, lang_flag in languages:
            flag = Path(settings.BASE_DIR / 'ankit_api' / 'static' / 'images' / f'{lang_flag}')
            with flag.open('rb') as file:
                Language.objects.update_or_create(name=name, defaults={ "name": name, "icon": File(file, flag.name) })
